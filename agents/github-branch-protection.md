...
The rules in force on a repository's default branch come from the branch rules listing, which returns them in one answer with the source of each. Take from it whether any rule is in force, the required approval count, `dismiss_stale_reviews`, the required status check contexts, the code owner review requirement, the force push and deletion permissions, and the signed commit requirement. Take the archived flag and the pushed-at date from the repository's own record, and take the bypass actor list from the ruleset's own definition at the ruleset id that listing names. A branch's own classic protection record enforces alongside any ruleset there without appearing among that listing's rules: where one exists, take its `enforce_admins` state and its pull request review bypass allowances the same way.

Read which of those rules came from above the repository off the source type, the source and the ruleset id that same listing carries on every rule it returns, because a rule sourced above the repository applies to the default branch of every repository it names and a rule set resolved from the repository alone over-reports by exactly the repositories one covers. Every rule that listing returns is in force together and the most restrictive outcome wins. A ruleset's bypass actors and a classic protection record's bypass allowances each bypass only their own source's rule, so neither weakens a rule the other source puts in place. The branch rules listing and the branch's own classic protection record together are the whole of what is in force on a branch: where both come back empty, nothing is in force, and that is the answer rather than a read to go and repeat against a third path. The classic protection record read answers 404 on a branch with no classic protection configured: that 404 is the answer, not a failed read, and it reads as empty for this comparison.
...
Stop here if every default branch on a repository that is not archived carries a rule from that listing with at least one required approval, no bypass actor able to reach that rule, and the force push and deletion permissions both disabled, and where the branch also carries a classic protection record, that record's own `enforce_admins` is also true and its pull request review bypass allowances name no one. Report the repository count with the archived repositories and their pushed-at dates, whether any rule is in force on each default branch, with the source type, the source and the ruleset id on the rules in force there and the repositories each source reaches, whether the branch also carries a classic protection record and that record's own `enforce_admins` state, the `enforce_admins` and `dismiss_stale_reviews` states as counts, the bypass actors by name per repository including any classic protection bypass allowances, the required status check contexts per repository as strings, unmatched at this stage against the workflow files that would produce them, and the signed commit requirement and code owner review requirement as counts, and which of the repository, rule, ruleset and classic protection reads above answered and which did not, naming each one that did not rather than counting it as zero, and end.
...
Resolve the repository administrator list where the rule carries a bypass actor, or where a classic protection record's own `enforce_admins` is false: that list and the bypass actors, the classic protection bypass allowances among them, together decide who can merge past the rule.
...
Branch rules: `GET /repos/{owner}/{repo}/rules/branches/{branch}` returns the rules in force on a branch, each carrying the source type, the source and the ruleset id that put it there; `GET /repos/{owner}/{repo}/rulesets` names the rulesets on a repository by id, and `GET /repos/{owner}/{repo}/rulesets/{ruleset_id}` returns one ruleset's own definition with its rules and its bypass actors. `GET /repos/{owner}/{repo}/branches/{branch}/protection` returns a branch's own classic protection record where one exists, with its own `enforce_admins` state and pull request review bypass allowances. All on `api.github.com`.
