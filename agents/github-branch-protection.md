...
Resolve the repository administrator list where the rule's `enforce_admins` is false, or where a classic protection record's own `enforce_admins` is false: that list and the bypass actors, the classic protection bypass allowances among them, together decide who can merge past the rule.
