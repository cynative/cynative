...
The rules in force on a repository's default branch come from the branch rules listing, which returns them in one answer with the source of each. Take from it whether any rule is in force, the bypass actor list, `enforce_admins`, the required approval count, `dismiss_stale_reviews`, the required status check contexts, the code owner review requirement, the force push and deletion permissions, the signed commit requirement, the archived flag and the pushed-at date.
...
Compare the required status check contexts against the job names the repository's workflows produce and report the contexts that no longer run: a required check that never reports satisfies the requirement vacuously.
...
Report an unprotected repository as intentional where the evidence supports it: an archived repository, a repository with a single contributor and no workflow, or a fork whose upstream carries the rule. Name the evidence. An unprotected default branch with no such evidence is not intentional.
