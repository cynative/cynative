...
Research which EC2 instances, load balancer interfaces, ECS tasks, EKS clusters, Lightsail instances, SageMaker notebooks and training jobs and EMR clusters in this account are reachable from the internet, and what the identity attached to each one holds. A managed database or analytics endpoint carries its own public-access flag; where a rule reported here is attached to one, name the endpoint the rule fronts, but that flag and the authentication behind it are outside this agent's scope.
