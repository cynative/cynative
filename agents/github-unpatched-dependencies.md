...
Read every repository's Dependabot security update status and its Dependabot alerts. A repository with the status disabled holds no alert rather than no vulnerability, so report it as unscanned and never as clean: the absence of an alert there is the absence of a scanner, and a repository nobody scans is the one a reader should look at first.
