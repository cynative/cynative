...
Stop here if every interface carrying a Basic SKU public IP address has a network security group at the interface or at its subnet, and either no inbound allow rule from `Internet`, `*`, `0.0.0.0/0`, `::/0` or a prefix outside RFC 1918, carrier-grade NAT, link-local and unique local space reaches TCP 22, 3389, 445, 1433, 3306, 5432 or 27017 or a UDP range outside 443, 500 and 4500, or no interface in any subnet those rules govern has a public IP address, a load balancer inbound NAT rule, or backend pool membership behind a load-balancing rule with a public frontend. Report the rule count by destination port range and by prefix length with the `Internet`, `*`, `0.0.0.0/0` and `::/0` rules counted apart from the other public prefixes, the TCP 80 rules as a count, the UDP rules with their destination port ranges, the public IP addresses with the resource each is attached to, the interfaces carrying a Basic SKU public IP address with a network security group at neither the interface nor its subnet as a count, the public IP addresses by SKU as a count, and the subnets with no network security group associated, and which of the rule, interface, address and subnet enumerations above answered and which did not, naming each one that did not rather than counting it as zero, and end.

Only for the rules and interfaces that are not clean:

An interface carrying a Basic SKU public IP address with no network security group at either level admits every inbound flow: report it as reaching its machine on every port, with whether that machine is running, and resolve that machine's identity below with the rest.
...
Report a subnet with no network security group as a finding only where an interface in it also has no group and carries a Basic SKU public IP, since a Standard SKU address stays closed without a group; otherwise leave the association gap in the count above.
