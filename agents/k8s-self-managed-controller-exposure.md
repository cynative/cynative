...
On a component whose `--secure-port` is not `0`, report `--bind-address` values other than `127.0.0.1` together with whether `--authentication-kubeconfig` and `--authorization-kubeconfig` are set on the same component, whether `--secure-port` carries a custom nonzero value, and with whether any NetworkPolicy restricts pod traffic to node addresses. A `--secure-port` of `0` disables the endpoint, and there is no port to report exposed. A bound address with neither kubeconfig set denies every path but `/healthz`, `/readyz` and `/livez` by default, so it exposes only those health paths on the node network rather than the metrics and debug endpoints; with both kubeconfigs set, the metrics and debug endpoints become reachable too, and the finding is their reachability rather than an open surface, since authentication and authorization still gate them. Report which of those two states each component is in rather than reporting the address alone.
