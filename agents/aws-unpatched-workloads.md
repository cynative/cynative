...
Read the support status the provider API reports for each: RDS engine versions marked deprecated or past support, EKS clusters past their Kubernetes end-of-support date, OpenSearch domains with an available service software update, Elastic Beanstalk environments without managed platform updates, and SSM patch compliance state with each instance's ping status and association state.

Read `AutoMinorVersionUpgrade` on ElastiCache Redis clusters together with the engine version. From Redis 6 the flag is inert because AWS applies minor upgrades regardless of it, so a disabled flag at engine version 6 or above describes a setting rather than a stale workload and produces a finding nobody can act on.
