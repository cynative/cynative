...
Read `clusterSecurityGroupId` under `resourcesVpcConfig` on each EKS cluster, which the cluster describe returns. It names the security group the control plane creates for the cluster and attaches to the cluster's own elastic network interfaces, and that a managed node group carries only where its launch template names no security groups of its own, so it is the boundary the EKS API states at the cluster level for pod-to-pod and node-to-control-plane traffic rather than a per-node fact; a cluster returning no value for it has none. What the cluster's NetworkPolicy objects say is not visible through the EKS API and is outside this agent's scope.
