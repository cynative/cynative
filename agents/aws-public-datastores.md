...
Stop here if no RDS instance or Redshift cluster has `PubliclyAccessible` set, no RDS instance sits outside a VPC, every OpenSearch domain has VPC options, no MSK cluster has public access, no DMS instance is marked public, no MQ broker has public accessibility and no AppSync API has a default authentication of `API_KEY`. Report the regions the sweep covered and the enabled-region listing it took them from, the counts by service, the OpenSearch domains with fine-grained access control disabled by name, and the MSK clusters permitting unauthenticated client access by name, and which of the seven service enumerations above answered and which did not, naming each one that did not rather than counting it as zero, and end.
