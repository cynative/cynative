...
Resolve each alert's action group and report alerts whose action group is not enabled or has no receiver, matching the gate's own criterion. Do the same for the subscription's security contact: report an empty contact address with the configured minimum alert severity and attack path risk level.
