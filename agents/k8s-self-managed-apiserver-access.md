...
Where `--authorization-mode` contains `AlwaysAllow` or the admission plugins contain `AlwaysAdmit`, report that as the single finding and end: every other argument describes a control that the authorizer or the admission chain has already been told to skip, so reporting them would describe protections that do not run.
...
Stop here if `--authorization-mode` includes both `RBAC` and `Node`, `--anonymous-auth` is false, no `--token-auth-file` is configured, `--service-account-lookup` is true and `--client-ca-file`, `--service-account-key-file`, `--tls-cert-file` and `--tls-private-key-file` are all set. Report the argument inventory, the `--enable-admission-plugins` and `--disable-admission-plugins` lists with whether each of `NodeRestriction`, `ServiceAccount`, `NamespaceLifecycle`, `DenyServiceExternalIPs`, `PodSecurity` and `SecurityContextDeny` is in force, and the `--audit-log-path` value, and end.
