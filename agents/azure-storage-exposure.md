...
Read six settings per account: `allowBlobPublicAccess`, `allowSharedKeyAccess`, the network rule set's `defaultAction`, `publicNetworkAccess`, `supportsHttpsTrafficOnly` and `allowCrossTenantReplication`, together with the customer-managed key configuration and the static website configuration. A `defaultAction` of `Allow` means the account answers from the internet, and the virtual network and IP rules alongside it do not narrow that - they are exceptions to a `Deny` default only.
