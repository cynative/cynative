...
Read the counts per namespace, over those same containers, of the ones that permit privilege escalation by setting `allowPrivilegeEscalation` true or leaving it unset, that add any capability, that retain `NET_RAW` by not dropping it, that do not drop `ALL`, that run as root because `runAsNonRoot` is false or unset and `runAsUser` is 0 or unset at both pod and container level, and that carry no `RuntimeDefault` seccomp profile at either level, in the same pass.
...
Stop here if the only containers setting `privileged`, `SYS_ADMIN`, `hostPath`, `hostPID`, `hostIPC`, `hostNetwork`, `hostPort` or `hostProcess` belong to DaemonSets from a CNI, CSI or monitoring component or to the static pods named above, and no container carries a literal value in place of a `secretKeyRef` in one of the environment variables named above. A static pod crosses that boundary by construction, so its boundary fields say nothing that is not true of every cluster of its kind. Report each of those DaemonSets and static pods with its image reference, its namespace and the host path and read-only flag on each `hostPath` mount it carries, the `automountServiceAccountToken` state per namespace with the service account name each pod runs as, and the counts per namespace for `allowPrivilegeEscalation` true or unset, added capabilities, `NET_RAW` retained, capability drops missing `ALL`, root containers by `runAsNonRoot` and `runAsUser`, and missing `RuntimeDefault` seccomp profiles, and end.
...
Report literal environment values with the variable names, the pods and the namespaces: they are readable by any subject that can read the pod specification.
