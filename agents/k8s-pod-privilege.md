...
Report each literal environment value and each literal `command` or `args` value by the variable or flag name, the pod and the namespace, and by what selected it rather than what it says: the variable or flag name that met the bound, and where the value itself matched a shape, which shape; never reproduce the value, in the script's output or in the report, since it is readable by any subject that can read the pod specification and this report must not become one more place it can be read.
