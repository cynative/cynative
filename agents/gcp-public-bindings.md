...
Stop here if no binding names either principal. Report the IAM policies named above as counts by the level each binding sits at, the resource counts split by principal into `allUsers` and `allAuthenticatedUsers`, the count of those bindings carrying a `condition`, the `constraints/iam.allowedPolicyMemberDomains` state with the projects and folders it applies at, the count of BigQuery datasets with no customer-managed key and Compute Engine disks with no customer-supplied key, the service enablement state for the Cloud Resource Manager API per project, and the count of projects the enumeration covered, and which of the policy and resource enumerations above answered and which did not, naming each one that did not rather than counting it as zero, and end.

Only for the resources that are not clean:

Decide what a binding grants from the `condition` it carries before treating it as exposure: a binding whose condition is false for the resource as you read it, because its expiry has passed or its resource clause does not match the resource it sits on, names the principal without granting access today, so report it as a conditioned binding with the condition named rather than as a public one. A condition you cannot resolve from what you read is unresolved rather than a grant, and say which part of it you could not settle.
...
For the datasets and buckets you reported above, read whether each has a customer-managed key. The caller reading data does not normally need `cloudkms.cryptoKeyVersions.useToDecrypt` itself: the storage or BigQuery service agent performs the decrypt, authorized by its own `roles/cloudkms.cryptoKeyDecrypter` or another role granting that permission on the key, while the caller is authorized by the bucket or dataset binding alone. Check that the service agent holds that grant, then assess the resource's data-read access on its own terms: a public bucket or dataset stays exposed under a customer-managed key just as it would without one, so report it as exposed rather than comparing it against the key's principals.
...
Cloud Resource Manager: `GET /v3/projects:search` on `cloudresourcemanager.googleapis.com`.
