...
Read the etcd pod's `--client-cert-auth`, `--auto-tls`, `--peer-client-cert-auth`, `--peer-auto-tls`, `--cert-file`, `--key-file`, `--peer-cert-file`, `--peer-key-file` and `--trusted-ca-file`, and the API server's `--etcd-cafile`, `--etcd-certfile`, `--etcd-keyfile`, `--client-ca-file` and `--encryption-provider-config`.
