...
Research what can read this cluster's etcd without passing through RBAC or the audit log, and whether an encryption provider is configured for the secrets stored there. Every argument below is read from the etcd and API server pods, so this agent applies to a self-managed control plane only: where a pod enumeration that answered returns no such pods, this cluster does not run etcd and the API server as pods. Through the `eks`, `gke` or `aks` connector that is the provider's control plane, and the answer is that this agent does not apply rather than a page of unresolved arguments. Through the `kubernetes` connector the API does not say whether the control plane is a provider's reached by kubeconfig or runs them as host processes, and its etcd access and encryption settings exist either way and are not readable here: say in one sentence that every argument below is unresolved for that reason, naming the enumeration that established it, and end. An enumeration that failed is neither answer and settles nothing: report it as unresolved rather than as an absent control plane. The API server's authorization and admission arguments and the kubelet arguments are outside this agent's scope.
