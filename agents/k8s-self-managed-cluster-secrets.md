...
Stop here if `--client-cert-auth` and `--peer-client-cert-auth` are both true with `--auto-tls` and `--peer-auto-tls` both false, its `--trusted-ca-file` differs from the API server's `--client-ca-file`, the API server presents `--etcd-certfile` and `--etcd-keyfile` with an `--etcd-cafile`, and `--encryption-provider-config` is set. Report the argument inventory with the `--listen-client-urls`, `--listen-peer-urls`, `--cert-file`, `--key-file`, `--peer-cert-file` and `--peer-key-file` values, and the `--encryption-provider-config` path with its providers marked unread, and end.
