...
Read `disablePasswordAuthentication` on every Linux machine, whether each machine uses managed or unmanaged disks, each machine's image reference and publisher, and the Azure Backup, endpoint protection and system update assessment states, taking all three from Defender for Cloud's assessment results, which carry one per machine and cost one call for the three. The per-machine patch route is an assessment this agent must not take: the operation that produces a fresh one runs on the machine. Read which role definitions include `Microsoft.Compute/virtualMachines/runCommand/action` together with their assignments.
