...
Read every service account's bindings at project, folder and organization level using Cloud Asset Inventory's organization-wide IAM policy search, together with its user-managed key count, and read the API target and application restrictions on every API key. Read whether the default Compute Engine and App Engine service accounts still hold `roles/editor` at project level.
