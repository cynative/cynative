...
The hop limit is the IP TTL on the metadata service's responses. At 1 the response cannot cross the extra hop into a container on a bridge network; above 1 it can, but only where no host firewall rule, ECS agent setting such as `ECS_AWSVPC_BLOCK_IMDS`, or equivalent network-namespace control blocks that path. The hop limit alone proves the packet can cross the hop, not that every container on the host receives the instance profile's credentials, whatever the token setting is.
