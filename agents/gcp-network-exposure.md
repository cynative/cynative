...
List ingress rules with a source range of `0.0.0.0/0` or `::/0` on TCP 22 or TCP 3389, and list the running instances with an external IPv4 or IPv6 address on a network interface. A rule admits that traffic only where it allows and is not disabled, and only where no matching rule with a lower `priority` number denies it first: GCP evaluates rules in `priority` order, lowest number first, so a deny blocks the traffic only where its `priority` number is no higher than a matching allow's, a higher-numbered deny does not override a lower-numbered allow, and a disabled rule admits nothing at all. Rules select instances by target tag, by target service account, or, where neither is set, every instance in the network; a rule may also carry `sourceTags` or `sourceServiceAccounts` on the source side, but GCP matches those against `sourceRanges` as a union rather than an intersection, so a rule with a source range of `0.0.0.0/0` stays open to the internet whether or not it also carries them.

Firewall rules attach to a VPC network rather than to an instance, so an instance with interfaces on two networks is governed by two different rule sets. Resolve rules per interface and report the network each conclusion belongs to. A firewall policy attached above the network, hierarchical at the organization or folder or a network firewall policy bound to the VPC, allows or denies the same traffic independently of the network's own rule list, so what reaches an interface is that interface's effective firewall rules, which come back with the policy rules and the network's rules together and already ordered as GCP applies them. A policy rule that neither allows nor denies but defers to the next tier settles nothing on its own, so take the answer from the tier it defers to rather than counting it as an allow. Settle reachability on that effective set, and where it does not answer, fall back to the network's own rules and say that the policy tiers above it went unread rather than concluding from the rule list alone.
...
Stop here if no ingress rule with a source of `0.0.0.0/0` or `::/0` reaches TCP 22 or TCP 3389 on a network whose instances carry an external IPv4 or IPv6 address, or if no running instance has an external IPv4 or IPv6 address. Report the rule count by source range and destination port with the rules that deny or are disabled - a deny overridden by a lower-numbered-priority allow noted apart from that count rather than folded into it - and the `35.235.240.0/20` rules counted apart from the rules that allow, each allowing rule's `sourceTags` or `sourceServiceAccounts`, if any, noted rather than treated as narrowing it, and each rule split by whether it selects by target tag, by target service account or by neither, the instance count with external IPv4 or IPv6 addresses by project and VPC network, one line per network interface, the OS Login and two-factor state both at project level and in instance metadata, and the count of projects the enumeration covered, the service enablement state for the Compute Engine API per project, and which of the rule, policy, instance and metadata reads above answered and which did not, naming each one that did not rather than counting it as zero, and end.
