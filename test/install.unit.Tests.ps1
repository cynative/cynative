...
"@
    ...
    It 'returns the single matching hash' {
        Get-CynExpectedHash -ChecksumsText $script:checksums -ArchiveName 'cynative_Windows_x86_64.zip' |
            Should -Be 'bbbb2222'
    }
    It 'throws when there is no entry' {
        { Get-CynExpectedHash -ChecksumsText $script:checksums -ArchiveName 'nope.zip' } | Should -Throw
    }
