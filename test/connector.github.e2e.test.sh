...
def _selftest():
    ...
    # DUMP_CORPUS=1 (re)populates test/lib/connector_audit/testdata/github/<name>/
    # from THIS pre-extraction parser; a normal --selftest run never writes it, only
    # reads github.names.txt for the name+code pin below.
    dump_root = os.environ.get("CONNECTOR_AUDIT_TESTDATA_DIR")
    dumping = bool(dump_root) and os.environ.get("DUMP_CORPUS") == "1"
    provider_dir = os.path.join(dump_root, "github") if dump_root else None
    if dumping:
        import shutil
    ...
    # the frozen testdata/github.names.txt exactly, so a case silently dropped, added,
    # or renumbered by a future refactor (e.g. the shared-harness extraction) is
    # caught here rather than passing on a shorter or drifted table.
    if dump_root:
        names_file = os.path.join(dump_root, "github.names.txt")
        if dumping:
