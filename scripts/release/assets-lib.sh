...
# manifest ("name<TAB>sha256<TAB>path" rows), failing closed unless EXACTLY one
# row names the asset and its digest is 64-hex (never publish or audit a
# Formula with a bad or ambiguous sha256).
sha_for() {
  local assets="$1" name="$2" sha count
  count="$(awk -F'\t' -v n="${name}" '$1==n{c++} END{print c+0}' "${assets}")"
  if [ "${count}" -ne 1 ]; then
    echo "::error::expected exactly 1 asset row for ${name}, found ${count}" >&2
    return 1
  fi
...
# EXACTLY one row names the asset and its digest is 64-hex. A separate function
# from sha_for because the input shapes differ (this one has no path column):
# the Scoop path must reject a duplicate row, because Scoop verifies the
# manifest SHA-256 and a wrong hash bricks every install.
sha_for_checksums() {
