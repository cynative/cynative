...
- Root `agents.go` uses `//go:embed all:agents` and `agents/` ships holding only `.keep`; the build needs both. `--agent` files come only from `~/.cynative/agents/` and that embedded tree. Never add a working-directory tier: a checkout must not be able to supply the run's prompt.
