...
- `make check-scripts` installs nothing. It needs shellcheck at the exact `SHELLCHECK_VERSION`, PowerShell 7 with the pinned Pester and PSScriptAnalyzer, python3, PyYAML and jq. Those pins sit at the top of the `Makefile` and are bumped by hand. It also needs openssl and go for `test/proxy.smoke.test.sh`, which builds the binary and mints a test CA; neither is version-pinned, just presence-checked by the suite itself. A new Python helper under `test/lib/` must be added to the syntax-gate list in the `sh-test` recipe or it ships with no syntax check.
...
- `test/proxy.smoke.test.sh` (wired into `sh-test`) is the only end-to-end proof of proxied routing and of the audit record's `route` field; `test/lib/connector_audit` ignores that field entirely. Its two fixtures, `test/lib/intercept-proxy.py` and `test/lib/fake-llm.py`, import only the standard library; keep them that way.
...
- `Run` in `internal/sandbox` cancels the run context before it closes `s.done`, and a worker that wins a concurrency slot re-reads that context inside the winning arm before it calls the tool. Both halves are load-bearing: a script can end its run with the context still live by throwing out of the IIFE wrapper, and the acquire select can have both arms ready at once, so neither signal alone says the run is over (#327).
...
- The host gates are port-blind by contract: the transport passes `AuthorizeHost` the view's `Hostname` (`authreq.NewView` lower-cases it once; `AuthorizeHost` normalizes nothing) and the dial guard sees only an IP, so every connector binds its own port in its action gate, via `authorizeRequestPort` (`internal/auth/authority.go`): `k8sGate.expectedPort` covers eks/gke/aks (443, the port their bootstrap fetch already assumes) and `kubernetes` (the kubeconfig port); `gitlab` passes its own configured port to the same helper; `github`, `aws`, `gcp` and `azure` call it directly with a fixed 443. An empty expected port denies rather than defaulting to 443. For `kubernetes` the identity the connector publishes is the authority the gate admits, both read from one parse of the kubeconfig server, so change them together and keep the host ASCII: an internationalized name lower-cases one way and IDNA-maps another, which would point the credential at a different host (#308).
- One rule admits a host: `auth.AdmitHost`, called by `transport.do` before anything lower-cases the request host, and by the `gitlab` and `kubernetes` connectors over their configured one. It refuses two shapes under two sentinels. A non-ASCII host (`ErrNonASCIIHost`) has three spellings that need not agree: Go's case mapping gives the one the gates classify, the IDNA conversion gives the one it dials, and the punycode conversion gives the one it writes into the `Host` header. Measure before you claim which two differ; U+0130 produces three different names, while U+212A folds and dials the same name and differs only on the header. An IP literal carrying a zone (`ErrZonedHost`) is ASCII and still has two, because the gates compare the zone lower-cased while Go resolves it through `zoneCache.index`, a case-sensitive lookup of interface names that falls back to reading the zone as a decimal index and hands the number to `SockaddrInet6.ZoneId`. The zone test is `netip.ParseAddr` returning a non-empty zone, the same test `net.Resolver.lookupIPAddr` applies, never a search for `%`: a percent also reaches a hostname as an escape that survived parsing, and that one names no interface. Any new host normalizer runs its ASCII scan on the caller's exact string, before `ToLower` or `TrimSpace`, as `cloudauth.NormalizeHost` does.
...
- A gate that segments a request path classifies every reading `authreq.View.PathReadings` returns and has to account for all of them: AWS authorizes the union of the operations they name, GCP requires the readings to agree on one method and denies when they do not. The servers disagree about whether the path is decoded before it is routed (Lambda and Google's frontend route it as sent, S3 decodes once and then splits), so a single reading is a guess; the wire reading `PathReadings` returns is the path exactly as sent, split only on the separators the wire carries, with no decoding. Four gates classify on one reading instead, each for a reason specific to its backend: `k8sgate` stays on the decoded path because kube-apiserver reads that same decoded path for both routing and RBAC; `internal/auth/azure/classifier.go` also segments the decoded path, safe only because `nonCanonicalPath` rejects an escaped %2f and any empty, ".", or ".." segment; `github` classifies the escaped path alone; `gitlab` splits the escaped path on literal '/' and percent-decodes each segment, never the whole path decoded.
...
- Outbound routing has one owner: `auth.Egress`, built by `auth.NewEgress` from `os.LookupEnv` in `newDeps` and nowhere else. A hand-built transport that carries a caller-supplied dial guard, such as the model-facing `http_request` transport, binds its route through `configureTransport` (`internal/auth/egress_route.go`): a direct route keeps the guard, a proxied route clears it and may dial only the proxy's canonical authority. An SDK or reusable client (`awsLoadOptions`, `withRefreshClient`, `withAPIClient`, `HTTPClient`) instead threads the same per-request selector, `Egress.proxyForRequest`, onto its own transport, with no guard to clear. Never set `http.ProxyFromEnvironment` and never read the proxy variables elsewhere; `https://` proxies and a `no_proxy` value with control characters are rejected at startup by design. Proxy credentials are scrubbed from transport errors and connector status reasons (`Scrub`), so an error rendered anywhere else must not carry them either.
...
- `connector-e2e.yaml` also carries a static `agent-e2e` job (`make agent-e2e`, `test/agent.e2e.test.sh`): a live run of a shipped built-in agent against the GCP fixture, a read plus a write canary, under the same golden. Its read phase is judged by `test/lib/agent-witness.py`, whose `--selftest` runs in `sh-test`; a new witness branch needs a case there, since the suite itself only runs live on the release path.
