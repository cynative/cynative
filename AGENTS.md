...
- The request host is admitted only as ASCII (`auth.ASCIIHost`, called in `transport.do` before anything lower-cases it). Go folds U+0130 to `i` and U+212A to `k` while it dials the IDNA form, so a folded host is classified under a name the wire never reaches. Any new host normalizer runs its ASCII scan on the caller's exact string, before `ToLower` or `TrimSpace`, as `cloudauth.NormalizeHost` does.
