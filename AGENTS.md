...
- The host gates are port-blind by contract: the transport passes `AuthorizeHost` the view's `Hostname` (`authreq.NewView` lower-cases it once; `AuthorizeHost` normalizes nothing) and the dial guard sees only an IP, so a connector whose endpoint can sit on a non-default port binds the port itself in its action gate, via `authorizeRequestPort` (`internal/auth/authority.go`). `k8sGate.expectedPort` covers eks/gke/aks (443, the port their bootstrap fetch already assumes) and `kubernetes` (the kubeconfig port); `gitlab` passes its own configured port to the same helper. An empty expected port denies rather than defaulting to 443. For `kubernetes` the identity the connector publishes is the authority the gate admits, both read from one parse of the kubeconfig server, so change them together and keep the host ASCII: an internationalized name lower-cases one way and IDNA-maps another, which would point the credential at a different host (#308).
