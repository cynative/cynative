...
- A gate that segments a request path classifies every reading `authreq.View.PathReadings` returns and authorizes the union. The servers disagree about whether the path is decoded before it is routed (Lambda and Google's frontend route it as sent, S3 decodes once and then splits), so a single reading is a guess; the wire reading `PathReadings` returns is the path exactly as sent, split only on the separators the wire carries, with no decoding. `k8sgate` is the exception and stays on the decoded path, because kube-apiserver reads the same decoded path for both routing and RBAC.
