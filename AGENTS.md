...
- Every outbound client resolves its target through `auth.RouteOutbound` (`internal/outbound` reads `HTTPS_PROXY`/`NO_PROXY` in the config loader): the request transport, the OpenAPI bootstrap fetches, the github token probe and the K8s/GitLab pinned clients. A direct route keeps the caller's dial-time address policy; a proxied one replaces it with a pin to the proxy endpoint, because the connection is to the proxy. A new outbound client goes through the same helper, never a bare `http.Transport`. The SDK-owned clients (aws-sdk-go-v2, the GCP token source, the metadata fetchers on `http.DefaultTransport`) already honour the same variables.
...
- `internal/outbound`: leaf; resolves the operator's `HTTPS_PROXY`/`NO_PROXY` into the per-target proxy decision every outbound client shares.
