...
A request can match more than one operation of a service. Cynative orders the matching templates by path the way the Smithy specificity routing rules do: a literal segment beats a label at the same position, a single-segment label beats a greedy one, and a longer template beats the shorter one it extends. Security Hub's `GET /automationrulesv2/list` therefore classifies as ListAutomationRulesV2 alone instead of tying with the `/automationrulesv2/{Identifier}` read. Templates whose paths rank equally are then separated by Cynative's own rule, the number of query and header discriminators the request carries, which counts the required member-bound parameters S3 routes on as well as the URI query literals Smithy counts. What is still tied after that is a real ambiguity: S3's `GET /` is both ListBuckets and ListDirectoryBuckets once the SDK-only `x-id` tag is ignored, a request that carries the sub-resource keys of two operations such as `?acl&tagging` matches both, and so does one that carries one operation's query literal and another's required header. Cynative then requires every candidate's actions to be allowed and denies if any candidate cannot be resolved, because nothing in the request says which operation AWS will run. Under SecurityAudit the plain bucket listing passes: the policy grants both `s3:ListAllMyBuckets` and `s3express:ListAllMyDirectoryBuckets`.

A path can also mean more than one thing. AWS services do not agree on whether a percent-encoded character in the path is decoded before the request is routed: Lambda binds an encoded slash inside the one segment it was sent in, while S3 decodes the path once and only then splits the bucket from the key. Cynative therefore classifies every reading of the path, the path as sent and the path decoded, and requires the IAM actions of every operation any reading names. A request whose readings name different operations is allowed only when the policy permits all of them, and a resource name that has to carry an encoded slash on the wire, such as an ARN in a path label, classifies from the reading that keeps it whole. A reading that ends in a trailing slash is also classified with that one empty segment dropped, so a path-style `GET /mybucket/` names the bucket listing alongside its own reading, matching S3's own answer to that request.
