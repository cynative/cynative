...
A request can match more than one operation of a service equally well. S3's `GET /` is both ListBuckets and ListDirectoryBuckets once the SDK-only `x-id` tag is ignored, and a request that carries the sub-resource keys of two operations, such as `?acl&tagging`, matches both. Cynative then requires every candidate's actions to be allowed and denies if any candidate cannot be resolved, because nothing in the request says which operation AWS will run. Under SecurityAudit the plain bucket listing passes: the policy grants both `s3:ListAllMyBuckets` and `s3express:ListAllMyDirectoryBuckets`. Templates are not yet ranked by path shape, so a request that fits both a literal segment and a label at the same position, or both a greedy label and a longer template after it, ties the same way and both candidates' actions must be allowed. In the shipped models SecurityAudit grants only one candidate of such a tie in a single case, the multi-region access point routes read on S3 Control, which is denied; a narrower policy has to grant both candidates. [#316](https://github.com/cynative/cynative/issues/316) tracks the ranking.
