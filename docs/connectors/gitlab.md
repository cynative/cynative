...
- **A glab OAuth refresh uses the instance's own glab settings.** cynative is HTTPS-only (its transport rejects any non-`https` URL), so an instance glab is configured to reach over plaintext `http` cannot be used through this connector regardless; the glab-side refresh follows the user's own glab `api_protocol`/`ca_cert` config. Use an `https` instance and a glab login that trusts it.
- **A mid-session `glab auth logout` or account switch takes effect at token expiry.** cynative caches the resolved glab token to avoid spawning a subprocess per request, so a local glab credential change is picked up when the OAuth token next needs a refresh (immediately for a non-expiring token only on the next session). Set `GITLAB_TOKEN` or restart cynative for an immediate change.
