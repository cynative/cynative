...
- Action authorization fails closed when service, operation, or permission resolution fails. Cynative derives an operation's required permission from its API method id, so a method whose IAM permission is named differently than its method id cannot be derived; it then resolves only if a pinned override or the high-confidence permission dataset supplies the answer, and is denied otherwise. The GKE Container control-plane reads are pinned for this reason: their permissions name the leaf resource (`container.clusters.*`, `container.operations.*`) while the method id nests it under `projects/{locations,zones}`, and a node-pool read authorizes on the parent cluster's `container.clusters.get`. Pinned and therefore resolvable: cluster and node-pool `get`/`list`, operation `get`/`list`, server config, Autopilot-compatibility and the cluster and node-pool upgrade-info reads, on both the `locations` and legacy `zones` routes. Not pinned, so still denied: the aggregated usable-subnetworks listing, which Google requires `container.clusters.create` for.
