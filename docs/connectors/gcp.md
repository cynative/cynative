...
- Action authorization fails closed when service, operation, or permission resolution fails. Cynative derives an operation's required permission from its API method id, so a method whose IAM permission is named differently than its method id cannot be derived. It then resolves only if a pinned override or the permission dataset supplies the answer, and is denied otherwise. The dataset is consulted in two tiers: writes use only the high-confidence entries, while reads additionally use the entries scraped from Google's per-parameter authorization block. That split is deliberate, because the scraped block is the complete documented answer for a read but is incomplete for a write. A read-tier answer is refused outright if any permission in it names a mutation (`create`, `delete`, `set*`, and the like), and the whole answer is discarded rather than filtered, so a partial answer can never under-require. That check is a denylist of known mutation verbs, not a proof: a permission whose verb is not on the list passes it, so treat it as defense in depth. The configured role is the actual boundary, and under a read-only role such as `roles/viewer` a write permission is refused there regardless. The GKE Container control-plane reads are the worked example: their permissions name the leaf resource (`container.clusters.*`, `container.operations.*`) while the method id nests it under `projects/{locations,zones}`, and a node-pool read authorizes on the parent cluster's `container.clusters.get`. Cluster, node-pool and operation `get`/`list` resolve through the read tier on both the `locations` and legacy `zones` routes; server config, Autopilot-compatibility and the cluster and node-pool upgrade-info reads are pinned. Not resolvable, so still denied: the aggregated usable-subnetworks listing, which Google requires `container.clusters.create` for.
- The read tier depends on an external dataset. Because the Container cluster, node-pool and operation reads resolve from it rather than from a pinned value, they are denied on a host that has never cached the dataset and cannot fetch it (an egress policy that permits `googleapis.com` but not `raw.githubusercontent.com`, for example). Once cached, the copy on disk is reused indefinitely if a later refresh cannot be **fetched**, so a network or status failure does not disturb an established install. A refresh that returns a malformed payload is not covered by that fallback: the parse failure discards the result without reverting to the cached copy, so those reads deny until a later refresh succeeds.
- The read tier can require a different read permission than you would expect. It reports Google's documented permission, which is often the parent resource's rather than the resource named in the method id: `apigateway.projects.locations.gateways.list` documents `apigateway.locations.get`, and `cloudbuild.projects.triggers.get` documents `cloudbuild.builds.get`. Under a broad ceiling such as `roles/viewer` this makes no difference, since the role grants both permissions. Under a narrow custom role it can matter: the role may grant the documented permission while withholding the one you intended to gate on. If you configure a tight custom role, check it against the permissions the operations you expect actually document.
- GCP read coverage is partial. Google does not publish per-method IAM requirements in its Discovery documents, so cynative resolves them by derivation and by an external permission dataset; the reads that resolve are a subset of the reads that exist, and the rest fail closed. Coverage is widest on the mainstream infrastructure APIs and thinnest on newer and niche ones.
- Authorization is per operation, not per response field. Some Google APIs gate individual response fields behind an extra permission: a GKE cluster `get`/`list` returns credential fields only to a caller holding `container.clusters.getCredentials`, which `roles/viewer` does not grant. Cynative checks the operation's documented permission against the configured role, so when the ambient ADC principal is broader than that role, a response can carry fields the role alone would not surface. Response redaction removes secret-shaped content before the model sees it, but it is defense-in-depth rather than an equivalent of the role ceiling.
